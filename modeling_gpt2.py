...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None,  layer_num:Optional[int]=None, prev_exp_sum : Optional[torch.FloatTensor] = None,
        ...
        # attn_weights_actual = nn.functional.softmax(attn_weights, dim=-1)
        attn_weights_actual = Softmax_written(attn_weights, dim=-1)
        ...
        if head_list is not None:
            if layer_num in head_list.keys() and len(head_list[layer_num])!=0:
                ...
                attention_probs = torch.empty_like(attn_weights_actual)
                # loss_local = torch.tensor(0.0).to('cuda')
                # loss_local.requires_grad = True
                ...
                for head_num_approx in range(attention_probs.shape[1]):
                    ...
                    if head_num_approx in head_list[layer_num]:
                        ...
                        if layer_num in skip_list.keys():#((layer_num==2)and(i==8))or((layer_num==1)and(i==0))or((layer_num==11)and(i==0))or((layer_num==11)and(i==5))or((layer_num==2)and(i==6))or((layer_num==6)and(i==0)):#use_prev[i]:
                            ...
                            attention_scores_inter = (attn_weights[:,head_num_approx,:,:]).reshape(attn_weights.shape[0],1,attn_weights.shape[2],attn_weights.shape[3])
                            ...
                            # attention_scores_inter =  (attention_scores_inter*attention_mask).reshape(attn_weights.shape[0],1,attn_weights.shape[2],attn_weights.shape[3])
                            ...
                            # x_inter_2 = x_inter.expand(attn_weights.shape[0],16,attn_weights.shape[2],attn_weights.shape[3]).clone()
                            # for ic in range(16):    
                        ...
                        if (layer_num in skip_list.keys()) and (i in skip_list[layer_num]):#((layer_num==2)and(i==8))or((layer_num==1)and(i==0))or((layer_num==11)and(i==0))or((layer_num==11)and(i==5))or((layer_num==2)and(i==6))or((layer_num==6)and(i==0)):#use_prev[i]:
                            ...
                            attention_probs[:,head_num_approx,:,:] =attn_weights_actual[:,head_num_approx,:,:]
                ...
                for i in range(12):
                    if (layer_num in skip_list.keys()) and (i in skip_list[layer_num]):#((layer_num==2)and(i==8))or((layer_num==1)and(i==0))or((layer_num==11)and(i==0))or((layer_num==11)and(i==5))or((layer_num==2)and(i==6))or((layer_num==6)and(i==0)):#use_prev[i]:
                        ...
                        attention_probs[:,i,:,:] = attn_weights_actual[:,i,:,:].clone()
        ...
        if head_list is None:
            attention_probs = torch.empty_like(attn_weights_actual)
            if layer_num == layer_num_approx:
                attention_probs = attn_weights_actual.clone()
                ...
                # attention_probs[:,head_num_approx,:,:] = attention_probs_inter.reshape(attention_probs_inter.shape[0],attention_probs_inter.shape[2],attention_probs_inter.shape[3])
                attention_probs[:,head_num_approx,:,:] = torch.zeros_like(attn_weights_actual[:,head_num_approx,:,:])
                loss_local = torch.tensor(0.0).to('cuda')
                loss_local.requires_grad = True
        ...
        # loss_local = nn.MSELoss()(attention_probs,attn_weights_actual)



        
        # print("Attention Weights {}".format(attn_weights[0,0,:,:]))
        # print(attn_weights.shape)
        # actual_softmax = nn.functional.softmax(attn_weights, dim=-1)
        ...
        # # print(torch.isinf(attn_weights).any())
        # # print("Min {}".format(torch.min(attn_weights)))
        # # print("Max {}".format(torch.max(attn_weights)))
        # attn_weights = self.softmax(attn_weights)
        # # print(torch.isinf(attn_weights).any())
        # # print(torch.isnan(attn_weights).any())
        # attn_weights = attn_weights.reshape(attn_weights.shape[0]//12,12,attn_weights.shape[2],attn_weights.shape[3])

        # loss = nn.MSELoss()(torch.sum(attn_weights,dim=-1),torch.sum(actual_softmax,dim=-1))
        # loss = nn.MSELoss()(attn_weights,attn_weights)
        ...
        # print(torch.isinf(attn_output).any())
        # print(torch.isinf(attn_output).any())
        # print(loss)
        # print(loss.dtype)
        return attn_output, attn_weights, prev_exp_sum_new
    ...
    def forward(
        ...
        if self.reorder_and_upcast_attn:
            ...
            attn_output, attn_weights, prev_exp_sum_new = self._attn(query, key, value, attention_mask, head_mask, layer_num=layer_num, prev_exp_sum=prev_exp_sum, head_list=head_list,skip_list=skip_list)
            # print(torch.isinf(attn_output).any())
        ...
        return outputs, prev_exp_sum_new  # a, present, (attentions)
...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        attn_outputs,prev_exp_sum_new = self.attn(
            hidden_states,
        ...
        # residual connection
        # print(torch.isinf(attn_output).any())
        # print(torch.isinf(residual).any())
        ...
        return outputs, prev_exp_sum_new  # hidden_states, present, (attentions, cross_attentions)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        transformer_outputs, loss_local = self.transformer(
            input_ids,
            past_key_values=past_key_values,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
        ...
        # print("from self.transformer {}".format(loss))
