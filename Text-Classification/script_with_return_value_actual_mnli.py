...
for j in range(12):
    head_list[j] = list()
x = "/scratch/gilbreth/amohanpa/bert-base/mnli/"
# x = "/scratch/gilbreth/amohanpa/bert-base/qqp/"
# x = "/scratch/gilbreth/amohanpa/bert-base/sst2/16channelModel/WithInit/16ChannelNoMask/head-by-head-seq_6_3_initfixed/"
# x = "/scratch/gilbreth/amohanpa/bert-base/qqp/16channelModel/WithInit/16ChannelNoMask/head-by-head-seq_2_11_initfixed/"
...
# print(sorted_error_dict)
# head_list = {0: [10, 11, 6], 1: [9], 2: [2, 6, 7, 10, 9, 11], 3: [3, 8, 5, 10, 2, 6, 9, 0], 4: [4, 9, 6, 1, 2], 5: [0, 5, 11, 9], 6: [11, 0, 8, 3], 7: [5, 10, 3, 6, 2, 4, 0], 8: [5, 10, 0, 9], 9: [9, 10, 5, 7, 8, 4], 10: [10, 1, 0, 11, 8, 7, 4, 6, 5, 2], 11: [8, 4, 9, 7, 1, 0, 10, 6, 5]}
# head_list = {0: [7], 1: [], 2: [7, 10, 9, 11], 3: [3], 4: [], 5: [], 6: [], 7: [], 8: [], 9: [9], 10: [1], 11: [8, 4, 9, 1]} #qqp
count = 0
for i in sorted_error_dict.keys():
    # if (count==0) and (i[0]!=2) and (i[1]!=11):
    #     continue
    # elif (count == 0) and (i[0]==2) and (i[1]==11):
    #     count=1
    #     continue
    if True:
        head_list[i[0]].append(i[1])
        print(head_list)
        y = "/scratch/gilbreth/amohanpa/bert-base/mnli/Simple/WithInit/Thresh080/TrainSize10/head-by-head-seq_{}_{}_initfixed/".format(i[0],i[1])
        try:
            ...
            result = subprocess.run(["python", "run_glue_no_trainer_actual.py", "--model_name_or_path", x , "--task_name", "mnli", "--seed", "42", "--max_length", "128", "--per_device_train_batch_size", "32", "--per_device_eval_batch_size", "128", "--gradient_accumulation_steps", "1", "--learning_rate", "0.187", "--with_tracking", "--num_train_epochs", "1", "--output_dir", y, "--layer0_list", "{}".format(head_list[0]), "--layer1_list","{}".format(head_list[1]),"--layer2_list","{}".format(head_list[2]),"--layer3_list","{}".format(head_list[3]),"--layer4_list","{}".format(head_list[4]),"--layer5_list","{}".format(head_list[5]),"--layer6_list","{}".format(head_list[6]),"--layer7_list","{}".format(head_list[7]),"--layer8_list","{}".format(head_list[8]),"--layer9_list","{}".format(head_list[9]),"--layer10_list","{}".format(head_list[10]),"--layer11_list","{}".format(head_list[11]),"--layer_num_new","{}".format(i[0]),"--head_num_new","{}".format(i[1])], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
