...
for i in sorted_error_dict.keys():
    ...
    if True:
        ...
        print(head_list)
        y = "/scratch/gilbreth/amohanpa/bert-base/mrpc/Simple/WithInit/Thresh055/TrainSize10/head-by-head-seq_{}_{}_initfixed/".format(i[0],i[1])
        try:
            ...
            result = subprocess.run(["python", "run_glue_no_trainer_actual.py", "--model_name_or_path", x , "--task_name", "mrpc", "--seed", "42", "--max_length", "128", "--per_device_train_batch_size", "32", "--per_device_eval_batch_size", "128", "--gradient_accumulation_steps", "1", "--learning_rate", "0.187", "--with_tracking", "--num_train_epochs", "1", "--output_dir", y, "--layer0_list", "{}".format(head_list[0]), "--layer1_list","{}".format(head_list[1]),"--layer2_list","{}".format(head_list[2]),"--layer3_list","{}".format(head_list[3]),"--layer4_list","{}".format(head_list[4]),"--layer5_list","{}".format(head_list[5]),"--layer6_list","{}".format(head_list[6]),"--layer7_list","{}".format(head_list[7]),"--layer8_list","{}".format(head_list[8]),"--layer9_list","{}".format(head_list[9]),"--layer10_list","{}".format(head_list[10]),"--layer11_list","{}".format(head_list[11]),"--layer_num_new","{}".format(i[0]),"--head_num_new","{}".format(i[1])], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
