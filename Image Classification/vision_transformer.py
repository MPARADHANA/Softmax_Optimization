...
def softmax_torch(x, dim = -1):
    ...
    max_static = torch.full_like(maxes,50.0)
    x_exp = torch.exp(x-max_static)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        #     nn.Unflatten(0,(-1,1)),
        #     nn.Conv2d(1,2,(1,16),padding='valid'),
        #     nn.ReLU(),
        # #   nn.Linear(512,1024),
        # #   nn.Hardsigmoid(),
        # #   nn.Flatten(0,1),
    ...
    def forward(self, x: torch.Tensor, layer_num, head_list: Optional[dict] =None, prev_attn: Optional[torch.Tensor]=None, use_prev:Optional[List[bool]]=[False,False,False]) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if self.fused_attn:
            ...
            # print(attn.shape)
            # attn_actual = attn.softmax(dim=-1)
            # attn_actual = nn.functional.softmax(attn,dim=-1)
            # print(attn_actual)
            attn_actual = softmax_torch(attn,dim=-1)
            attn_buf = attn_actual.clone()
            # loss_local = nn.MSELoss()(attn,attn)
            loss_local = torch.tensor(0.0).to('cuda')
            use_prev_new = [False,False,False]
            loss_local.requires_grad = True
            use_prev_new = [False,False,False]
            if prev_attn is not None:
                for i in range(3):
                    diff = nn.MSELoss()(attn_buf[:,i,:,:],prev_attn[:,i,:,:])
                    print("layer_num {} head {}".format(layer_num,i))
            if head_list is not None:
                
                # if layer_num in head_list.keys() and len(head_list[layer_num])!=0:
                #     for i in head_list[layer_num]:
                #         # attention_scores_inter = attn[:,i,:,:].reshape(attn.shape[0],attn.shape[2],attn.shape[3])
                #         attention_scores_inter = attn[:,i,:,:].reshape(attn.shape[0],1,attn.shape[2],attn.shape[3])
                #         x_inter = attention_scores_inter.clone()
                #         x_inter_2 = x_inter.expand(attn.shape[0],16,attn.shape[2],attn.shape[3]).clone()
                #         for ic in range(16):
                #             x_inter_2[:,ic,:,:]= torch.roll(x_inter_2[:,ic,:,:],shifts=(-ic),dims=(2))
                #         attention_probs_inter = self.softmax_16[i](x_inter_2)
                #         # attention_probs_inter = self.softmax[i](attention_scores_inter)
                #         attn[:,i,:,:] = attention_probs_inter.reshape(attention_probs_inter.shape[0],attention_probs_inter.shape[2],attention_probs_inter.shape[3])
                for i in range(3):
                    if ((layer_num==6)and(i==0))or((layer_num==10)and(i==2)):#or((layer_num==7)and(i==0))or((layer_num==10)and(i==0))or((layer_num==11)and(i==0))or((layer_num==9)and(i==1))or((layer_num==10)and(i==1))or((layer_num==10)and(i==2))or((layer_num==9)and(i==2)): #use_prev[i]:
                        attn_buf[:,i,:,:] = prev_attn[:,i,:,:]
                        
                        use_prev_new[i] = use_prev[i]  
                    elif layer_num in head_list.keys() and (i in head_list[layer_num]): 
                        attention_scores_inter = attn[:,i,:,:].reshape(attn.shape[0],1,attn.shape[2],attn.shape[3])
                        x_inter = attention_scores_inter.clone()
                        # x_inter_2 = x_inter.expand(attn.shape[0],16,attn.shape[2],attn.shape[3]).clone()
                        # for ic in range(16):
                        #     x_inter_2[:,ic,:,:]= torch.roll(x_inter_2[:,ic,:,:],shifts=(-ic),dims=(2))
                        attention_probs_inter = self.softmax_16[i](x_inter)
                        # attention_probs_inter = self.softmax[i](attention_scores_inter)
                        attn_buf[:,i,:,:] = attention_probs_inter.reshape(attention_probs_inter.shape[0],attention_probs_inter.shape[2],attention_probs_inter.shape[3])
                        # loss_local= loss_local + nn.MSELoss()(attn[:,i,:,:],attn_actual[:,i,:,:])
                    else:
                        attn_buf[:,i,:,:] = attn_actual[:,i,:,:] 
            # loss_local = nn.MSELoss()(attn,attn)
            
            
            if head_list is None:
                attn_buf = torch.empty_like(attn_actual)
                for i in range(3):
                    #if ((layer_num==4)and(i==1))or((layer_num==6)and(i==0))or((layer_num==7)and(i==0))or((layer_num==10)and(i==0))or((layer_num==11)and(i==0))or((layer_num==9)and(i==1))or((layer_num==10)and(i==1))or((layer_num==10)and(i==2))or((layer_num==9)and(i==2)): #use_prev[i]:
                    if ((layer_num==6)and(i==0))or((layer_num==10)and(i==2)):#or((layer_num==7)and(i==0))or((layer_num==10)and(i==0))or((layer_num==11)and(i==0))or((layer_num==9)and(i==1))or((layer_num==10)and(i==1))or((layer_num==10)and(i==2))or((layer_num==9)and(i==2)): #use_prev[i]:
                        attn_buf[:,i,:,:] = prev_attn[:,i,:,:]
                        
                        use_prev_new[i] = use_prev[i]
                    else: 
                        print("layer {}' head {}'".format(layer_num,i))
                        attn_buf[:,i,:,:] = attn_actual[:,i,:,:]
                        if prev_attn is not None:
                            
                            diff = nn.MSELoss()(attn_buf[:,i,:,:],prev_attn[:,i,:,:])
                            print(diff.item())
                            # use_prev_new[i] = diff.item() < 0.0001
                            # print(use_prev_new)
                        else:
                            use_prev_new = [False,False,False]
                ...
                #     print(head_num)
                #     print(nn.MSELoss()(attn[:,head_num,:,:], prev_attn[:,head_num,:,:]))

            # print(loss_local)
            # loss_local = torch.tensor(0.0).to('cuda')
            # loss_local.requires_grad = True
            attn_actual = attn_buf.clone()
            attn_buf = self.attn_drop(attn_buf)
            x = attn_buf @ v

        x = x.transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, loss_local, attn_actual, use_prev_new
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, layer_num: int, head_list: Optional[dict] =None,prev_attn:Optional[torch.Tensor]=None, use_prev:Optional[bool]=False) -> torch.Tensor:
        # print(type(x))
        # print(x)
        a = self.attn(self.norm1(x),layer_num=layer_num, head_list=head_list,prev_attn=prev_attn, use_prev = use_prev)
        loss_local = a[1]
        attn = a[2]
        use_prev = a[3]
        x = x + self.drop_path1(self.ls1(a[0]))
        # x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),layer_num=layer_num, head_list=head_list)[0]))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        # print(type(x))
        # print(x)
        return x, loss_local, attn, use_prev
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        print("Block fn {}".format(block_fn))
        self.feature_info = [
    ...
    def forward_intermediates(
        ...
        loss_local = None
        attn_actual = None
        use_prev=[False,False,False]
        for i, blk in enumerate(blocks):
            x,loss,attn_actual, use_prev = blk(x,layer_num=i,head_list=head_list,prev_attn=attn_actual,use_prev=use_prev)
    ...
    def forward_features(self, x: torch.Tensor, head_list:Optional[dict] = None) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
    ...
    def forward(self, x: torch.Tensor, head_list: Optional[dict]=None) -> torch.Tensor:
        x, loss_local = self.forward_features(x, head_list)
...
def vit_small_patch16_18x2_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    Paper focuses on 24x2 + 48x1 for 'Small' width but those are extremely slow.
    """
    print("block fn ParallelThingsBlock")
    model_args = dict(
...
def vit_base_patch16_xp_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    print("block fn ParallelScaling")
    model_args = dict(
...
def vit_large_patch14_xp_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Large model (ViT-L/14) w/ parallel blocks and qk norm enabled.
    """
    print("block fn Parallel Scaling")
    model_args = dict(
