...
class Trainer:
    ...
    def __init__(
        ...
        loss_coefficient: Optional[float] = 0.0,
        ...
        self.layer_num_approx = layer_num_approx
        self.head_num_approx = head_num_approx
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            ...
            if 'bert' in model.config.architectures[0].lower():
                loss, loss_local = self.compute_loss(model, inputs)
                loss = loss + self.loss_coefficient*loss_local
            else:
                loss= self.compute_loss(model, inputs)

            # print("Loss_local {}".format(loss_local))
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        # outputs, loss_local = model(**inputs)
        if ('bert' in model.config.architectures[0].lower()) or ('gpt2' in model.config.architectures[0].lower()) :
            # outputs, loss_local = model(**inputs,layer_num_approx = self.layer_num_approx,head_num_approx = self.head_num_approx, head_list = self.head_list)
            outputs, loss_local = model(**inputs,head_list = self.head_list, skip_list = self.skip_list)
            # print("loss Local from model {}".format(loss_local))
        elif ('whisper' in model.config.architectures[0].lower()): 
            outputs = model(**inputs,head_list = self.head_list)
        else:
            # outputs= model(**inputs, head_list=self.head_list)
        ...
        if 'bert' in model.config.architectures[0].lower():
            ...
            # print(loss_local)
            return (loss, outputs, loss_local) if return_outputs else (loss, loss_local)
        else:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if 'bert' in model.config.architectures[0].lower():
                loss, logits, labels,loss_local = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
                loss_local_mean += loss_local.item()
            else:
                loss, logits, labels= self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
        ...
        print("Loss_local_Mean {}".format(loss_local_mean))
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    # print("Inside")
                    with self.compute_loss_context_manager():
                        if ('bert' in model.config.architectures[0].lower()):# or ('whisper' in model.config.architectures[0].lower()):
                            loss, outputs, loss_local = self.compute_loss(model, inputs, return_outputs=True)
                        else:
                ...
                # print("Loss local {}".format(loss_local))
        ...
        if len(logits) == 1:
            logits = logits[0]
        if 'bert' in model.config.architectures[0].lower():
            return (loss, logits, labels, loss_local)
        else:
