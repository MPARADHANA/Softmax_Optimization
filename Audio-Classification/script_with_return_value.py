...
for i in error_dict.keys():
    if True:
        ...
        print(head_list)
        y = "/scratch/gilbreth/amohanpa/Whisper-tiny/common_language/Simple/WithInit/Thresh168/head-by-head-seq_{}_{}_initfixed/".format(i[0],i[1])
        try:
            ...
            result = subprocess.run(["python", "run_audio_classification.py", "--model_name_or_path", x , "--dataset_name", "speechbrain/common_language", "--num_train_epochs", "1", "--output_dir", y, "--per_device_train_batch_size", "32", "--gradient_accumulation_steps", "2", "--per_device_eval_batch_size", "64", "--logging_steps", "25", "--learning_rate", "0.187", "--warmup_steps", "0", "--dataloader_num_workers", "1", "--label_column_name", "language", "--do_eval", "--do_train", "--overwrite_output_dir", "--remove_unused_columns", "False", "--attention_mask", "False", "--layer0_list", "{}".format(head_list[0]), "--layer1_list","{}".format(head_list[1]),"--layer2_list","{}".format(head_list[2]),"--layer3_list","{}".format(head_list[3]), "--layer_num_new","{}".format(i[0]),"--head_num_new","{}".format(i[1])], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
