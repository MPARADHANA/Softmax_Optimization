...
def main():
    ...
    if model_args.layer0_list is not None:
        layer0_list_converted = ast.literal_eval(model_args.layer0_list)
        layer1_list_converted = ast.literal_eval(model_args.layer1_list)
        layer2_list_converted = ast.literal_eval(model_args.layer2_list)
        layer3_list_converted = ast.literal_eval(model_args.layer3_list)
        # layer4_list_converted = ast.literal_eval(model_args.layer4_list)
        # layer5_list_converted = ast.literal_eval(model_args.layer5_list)
        # layer6_list_converted = ast.literal_eval(model_args.layer6_list)
        # layer7_list_converted = ast.literal_eval(model_args.layer7_list)
        head_list = dict()
    ...
    skip_list= {}
    # head_list = {0: [4], 1: [4], 2: [0, 1, 3, 4], 3: [0, 2, 3, 4, 5], 4: [], 5: [], 6: [], 7: [], 8: [], 9: [], 10: [], 11: []}
            ...
            if m.weight.requires_grad:
                ...
                for name, module in model.named_modules():
                    ...
                    if module is m:
                        ...
                        layer_name = name
                        break
                # .encoder.layers.1.self_attn.softmax_16.4
                if model_args.layer_num_new > 3:
                    expected = "decoder.layers.{}.self_attn.softmax_16.{}".format((model_args.layer_num_new - 4),model_args.head_num_new)
                else:
                    expected = "encoder.layers.{}.self_attn.softmax_16.{}".format((model_args.layer_num_new),model_args.head_num_new)
                # "layer.{}.attention.self.softmax_16.{}".format(model_args.layer_num_new,model_args.head_num_new)
    ...
    if model_args.layer_num_new is not None:
        model.apply(init_weights)
    ...
    if training_args.do_train:
        ...
        for name, param in model.named_parameters():
            ...
            if 'softmax' in name:
                param.requires_grad = True
                print("layer where grad is set true")
                print(name)
                # print(name)
            #     # if '3.weight' in name:
            #     #     print("in 3.weight")
        ...
        loss_value = train_result.metrics["train_loss"]
        trainer.save_model()
